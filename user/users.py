...
def delete(name):
    try:
        ...
        if(name in names):
            print("username='"+name+"'")
            requests.post('http://orchestrator:12345/api/v1/db/write', json={"insert": "username='"+name+"'","column":"","table":"users","what":"delete"})
            res = jsonify()
            # res.statuscode = 201
            return res, 200
        else:
...
def list_all_users():
    # try:
        ...
        f.write(str(count))
        f.close()
        users = requests.post('http://orchestrator:12345/api/v1/db/read', json={"table":"users","columns":"username","where":"username!='wdjfuhwiufhwihfhwjhf'"})
        users = users.json()
